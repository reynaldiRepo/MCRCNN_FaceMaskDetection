        ...
        for jk in range(R.shape[0]//C.num_rois + 1):
            ...
            for ii in range(P_cls.shape[1]):
                ...
                if np.max(P_cls[0, ii, :]) < bbox_threshold or np.argmax(P_cls[0, ii, :]) == (P_cls.shape[2] - 1):
                    # print("Box Under Treshold", np.max(P_cls[0, ii, :]))
                    continue
...
def setupmodel(type="MCRCNN", doCompile= False):
    #rcnn model config
    if (type == "MCRCNN"):
        #MCRCNN model config
        configpath = os.path.join(os.getcwd(), "model", "model_config_MCRCNN.pickle")
...
def testALL(model="MRCNN", verbose=False):
    ...
    for k in all_dets :
        with open(os.path.join(os.getcwd(), "npy", "test_result_RCNN_"+str(k)+".npy"), "wb") as f:
            np.save(f, np.array(all_dets[k], dtype="float32"))
        f.close()
        with open(os.path.join(os.getcwd(), "npy", "test_result_RCNN_"+str(k)+".npy"), "rb") as f:
            b = np.load(f)
            print (b.shape)
        f.close()
    print(all_dets)
...
# /for predict
def doPredict(C, model_rpn, model_classifier, img):
    class_mapping = C.class_mapping
    class_mapping = {v: k for k, v in class_mapping.items()}
    # print("Class Mapping : ", class_mapping)
    # # class_to_color = {class_mapping[v]: np.random.randint(0, 255, 3) for v in class_mapping}
